...
const initiateVideoProcessingJobSchema = z.object({
  ...
  // targetVoices: z.array(z.string()).min(1),
    ...
    async ({
      ...
      try {
        ...
        if (jobCheckError) {
          ...
          // Proceed cautiously, may trigger duplicate download
        }
        ...
        if (
          ...
          downloadJobId = existingJob.id; // Capture existing job ID
          // The existing job completion will trigger the Supabase function
        } else {
          ...
          if (insertJobError) {
            console.error("Error inserting new download job:", insertJobError);
            // If inserting the job fails, we can't proceed with download trigger
            throw appErrors.DATABASE_ERROR;
          }

          downloadJobId = newJobId;

          // Fire-and-forget call to downloader service
          fetch(`${downloaderServiceUrl}`, {
            method: "POST",
            ...
            // Log error, status remains pending
          });
        }
        ...
        return {
          ...
          data: {
            videoId: videoId,
            downloadJobId: downloadJobId, // Return the relevant job ID
