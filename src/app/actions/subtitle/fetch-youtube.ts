...
export const fetchYouTubeSubtitles = publicAction
  .schema(fetchYouTubeSubtitlesSchema)
  .action(
    async ({
      parsedInput,
    }): Promise<ActionResponse<FetchYouTubeSubtitlesOutput>> => {
      const { youtubeUrl, languageCode } = parsedInput;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (
            ...
            message =
              "No subtitles found for the selected language on YouTube.";
